...
const AdminDashboard = () => {
  ...
  // FUNCIÓN QR CORREGIDA (Carga la imagen antes de imprimir)
  const generarQRImprimible = (path, titulo) => {
      ...
      {activeTab === 'admin' && (
        <div className="space-y-10 animate-in fade-in text-left">
          ...
          <section className="bg-slate-900 p-6 rounded-[2rem] border border-slate-800 shadow-2xl">
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto pr-2 custom-scroll text-left">
              {personal.map(p => (
                <div key={p.dni} className="p-3 bg-slate-950 rounded-xl border border-slate-800 flex justify-between items-center group shadow-md">
                   ...
                   <button onClick={async () => { if(window.confirm("¿Dar de baja?")) { await supabase.from('personal').delete().eq('dni', p.dni); cargarDatos(); } }} className="text-red-500 text-[10px] font-bold md:opacity-0 md:group-hover:opacity-100 transition-opacity uppercase shrink-0">Eliminar</button>
                </div>
              ))}
            </div>
          </section>
                ...
                <div key={p.id} className="bg-slate-950 p-6 rounded-3xl border border-slate-800 shadow-lg text-left">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => generarQRImprimible(`/piso/${p.slug}`, `PAÑOL - ${p.nombre_piso}`)} className="px-4 py-2 bg-slate-800 rounded-lg text-[9px] font-bold uppercase text-blue-500 border border-blue-900/30 hover:bg-blue-900/20">QR Pañol</button>
                      <button onClick={() => generarQRImprimible(`/lavadero/${p.slug}`, `LAVADERO - ${p.nombre_piso}`)} className="px-4 py-2 bg-slate-800 rounded-lg text-[9px] font-bold uppercase text-green-500 border border-green-900/30 hover:bg-green-900/20">QR Lavadero</button>
                      <button onClick={async () => { if(window.confirm(`¿Eliminar sector ${p.nombre_piso}?`)) { await supabase.from('pisos').delete().eq('id', p.id); cargarDatos(); } }} className="text-red-500 text-xl font-black ml-4 hover:scale-125 transition-all">×</button>
                    </div>
                  </div>
                  <div className="bg-slate-900/50 p-4 rounded-2xl border border-slate-800/50">
                    <div className="flex justify-between items-center">
                       <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic">Habitaciones Especiales (Guardia)</p>
                       <button onClick={() => {
                          const nombre = prompt("Nombre de la Habitación (Ej: Medico Interno):");
                          if(nombre) { generarQRImprimible(`/habitacion/${p.slug}-${nombre.toLowerCase().replace(/ /g, '-')}`, `${nombre.toUpperCase()} - ${p.nombre_piso}`); }
                       }} className="bg-blue-600/20 text-blue-400 px-4 py-1.5 rounded-xl text-[9px] font-black uppercase border border-blue-600/30 hover:bg-blue-600 hover:text-white transition-all">+ Agregar Habitación</button>
                    </div>
                  </div>
                </div>
              ))}
