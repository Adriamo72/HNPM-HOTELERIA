      ...
      {activeTab === 'historial' && (
        <section className="space-y-8 animate-in fade-in">
          <div className="flex justify-between items-center">
            ...
            <button onClick={cargarDatos} className="text-[10px] bg-slate-800 px-4 py-2 rounded-xl font-black text-slate-400 border border-slate-700 hover:bg-slate-700 transition-colors">Actualizar</button>
          </div>
          ...
          {Object.keys(resumenStock).map((nombrePiso) => (
            <div key={nombrePiso} className="bg-slate-900 rounded-[3rem] border border-slate-800 overflow-hidden shadow-2xl mt-8">
              ...
              {/* HISTORIAL CON SCROLL DE 5 FILAS */}
              <div className="p-3 space-y-1 overflow-y-auto max-h-[350px] custom-scroll bg-slate-950/20">
                  ...
                  <div key={m.id} className="bg-slate-950/50 px-5 py-3 rounded-2xl border border-slate-800/50 flex items-center justify-between group hover:bg-slate-800 transition-all">
                    ...
                    <div className="flex items-center gap-6 md:gap-10">
                      ...
                      <div className="text-center min-w-[50px]">
                        ...
                        <p className="text-lg font-black text-red-500">-{m.retirado_sucio || 0}</p>
                      </div>
                    </div>

                    <div className="w-1/3 flex items-center justify-end gap-4">
                      <div className="text-right">
                        <p className="text-[10px] text-slate-300 font-black uppercase leading-tight tracking-tighter italic">
                          {m.pañolero?.jerarquia} {m.pañolero?.apellido} {m.pañolero?.nombre}
                        </p>
                        {m.enfermero && (
                          <p className="text-[9px] text-slate-500 font-bold uppercase mt-1 tracking-tighter italic">
