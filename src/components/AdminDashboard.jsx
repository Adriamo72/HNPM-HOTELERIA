...
const AdminDashboard = () => {
  ...
  // NUEVA FUNCIÓN DE QR (MANTIENE TU ESTILO DE IMPRESIÓN)
  const descargarQRPersonalizado = (path, titulo) => {
  ...
  return (
    <div className="p-4 md:p-8 bg-slate-950 min-h-screen text-slate-100 font-sans relative">
        ...
        <section className="space-y-8 animate-in fade-in">
          ...
          {Object.keys(resumenStock).map((nombrePiso) => (
            <div key={nombrePiso} className="bg-slate-900 rounded-[3.5rem] border border-slate-800 overflow-hidden shadow-2xl mt-8">
              <div className="bg-slate-800/40 px-8 py-4 border-b border-slate-800"><span className="text-lg font-black text-blue-400 uppercase tracking-widest">{nombrePiso}</span></div>
                  ...
                  return (
                    <div key={m.id} className="bg-slate-950/50 px-3 py-1.5 rounded-2xl border border-slate-800/50 flex items-center group hover:bg-slate-800 transition-all">
                      ...
                      <div className="w-[32%] flex items-center justify-end gap-3 border-l border-slate-800 pl-3">
                        <div className="text-right overflow-hidden"><p className="text-[9px] text-slate-300 font-black uppercase leading-tight truncate">OP: {m.pañolero?.jerarquia} {m.pañolero?.apellido}</p></div>
      ...
      {activeTab === 'admin' && (
        <div className="space-y-10 animate-in fade-in">
          <section className="bg-slate-900 p-6 rounded-[2rem] border border-yellow-600/30 flex justify-between items-center shadow-xl">
            <div className="max-w-[70%] text-yellow-500"><h3 className="text-sm font-black uppercase italic">Mando de Auditoría</h3><p className="text-[10px] text-slate-500 uppercase font-bold">Ajuste manual de stock</p></div>
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto pr-2 custom-scroll">
              {personal.map(p => (
                <div key={p.dni} className="p-3 bg-slate-950 rounded-xl border border-slate-800 flex justify-between items-center group shadow-md">
                   <span className="text-xs font-bold uppercase">{p.jerarquia} {p.apellido}, {p.nombre}</span>
                ...
                <div key={p.id} className="bg-slate-950 p-6 rounded-3xl border border-slate-800 shadow-lg">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => descargarQRPersonalizado(`/piso/${p.slug}`, `PAÑOL - ${p.nombre_piso}`)} className="px-4 py-2 bg-slate-800 rounded-lg text-[9px] font-bold uppercase text-blue-500 border border-blue-900/30">QR Pañol</button>
                      <button onClick={() => descargarQRPersonalizado(`/lavadero/${p.slug}`, `LAVADERO - ${p.nombre_piso}`)} className="px-4 py-2 bg-slate-800 rounded-lg text-[9px] font-bold uppercase text-green-500 border border-green-900/30">QR Lavadero</button>
                  ...
                  <div className="bg-slate-900/50 p-4 rounded-2xl border border-slate-800/50">
                    <div className="flex justify-between items-center">
                       ...
                       <button onClick={() => {
                          ...
                          if(nombre) { descargarQRPersonalizado(`/habitacion/${p.slug}-${nombre.toLowerCase().replace(/ /g, '-')}`, `${nombre.toUpperCase()} - ${p.nombre_piso}`); }
                       }} className="bg-blue-600/20 text-blue-400 px-4 py-1.5 rounded-xl text-[9px] font-black uppercase border border-blue-600/30">+ Agregar Habitación</button>
                    </div>
                  </div>
                </div>
              ))}
      ...
      {notificacion.visible && (
        <div className="fixed bottom-4 right-4 bg-blue-600 text-white px-6 py-3 rounded-2xl shadow-2xl animate-in slide-in-from-right font-black uppercase text-[10px] tracking-widest z-[100] border border-blue-400">
