        ...
        <section className="space-y-8 animate-in fade-in">
          ...
          {Object.keys(resumenStock).map((nombrePiso) => (
            <div key={nombrePiso} className="bg-slate-900 rounded-[3.5rem] border border-slate-800 overflow-hidden shadow-2xl mt-8">
              ...
              {/* HISTORIAL COMPACTO PARA CELULARES */}
              <div className="p-2 space-y-1.5 overflow-y-auto max-h-[350px] custom-scroll bg-slate-950/20">
                {movimientosAgrupados[nombrePiso]?.map((m) => {
                  const esSincro = !m.entregado_limpio && !m.egreso_limpio && !m.retirado_sucio;
                  return (
                    <div key={m.id} className="bg-slate-950/50 px-3 py-1.5 rounded-2xl border border-slate-800/50 flex items-center justify-between group hover:bg-slate-800 transition-all">
                      <div className="w-[22%]">
                        <p className="text-[10px] font-black text-white uppercase leading-tight">{m.item}</p>
                        <p className="text-[8px] text-blue-500 font-black uppercase mt-0.5 tracking-tighter">{formatearFechaGuardia(m.created_at)}</p>
                      </div>

                      <div className="flex-1 flex justify-around items-center px-4">
                        <div className="text-center min-w-[50px]">
                          <span className="text-[7px] text-green-500 font-black uppercase block">Ing</span>
                          <p className={`text-lg font-black text-green-500 ${!m.entregado_limpio && !esSincro ? 'opacity-10' : ''}`}>
                            {esSincro ? `S:${m.stock_fisico_piso}` : m.entregado_limpio > 0 ? `+${m.entregado_limpio}` : '-'}
                          </p>
                        </div>
                        <div className="text-center min-w-[50px]">
                          <span className="text-[7px] text-blue-400 font-black uppercase block">Ent</span>
                          <p className={`text-lg font-black text-blue-400 ${!m.egreso_limpio ? 'opacity-10' : ''}`}>
                            {m.egreso_limpio > 0 ? `-${m.egreso_limpio}` : '-'}
                          </p>
                        </div>
                        <div className="text-center min-w-[50px]">
                          <span className="text-[7px] text-red-500 font-black uppercase block">Suc</span>
                      ...
                      <div className="w-[28%] flex items-center justify-end gap-2 border-l border-slate-800 pl-2">
                        <div className="text-right">
                          <p className="text-[8px] text-slate-300 font-black uppercase leading-tight tracking-tighter">
                            {m.pañolero?.apellido}
                          </p>
                          {m.enfermero && (
                            <p className="text-[8px] text-slate-500 font-bold uppercase tracking-tighter italic">
                              REC:{m.enfermero?.apellido}
                            </p>
                          )}
                        </div>
                        {/* ICONO ELIMINAR SIEMPRE VISIBLE EN MÓVILES */}
                        <button 
      ...
      {activeTab === 'admin' && (
        <div className="space-y-10 animate-in fade-in">
          ...
          <section className="bg-slate-900 p-6 rounded-[2rem] border border-yellow-600/30 flex justify-between items-center shadow-xl">
            ...
            <button onClick={toggleAuditoria} className={`px-8 py-3 rounded-xl font-black text-[10px] uppercase shadow-lg ${auditoriaHabilitada ? 'bg-red-600 animate-pulse text-white' : 'bg-green-600 text-white'}`}>
