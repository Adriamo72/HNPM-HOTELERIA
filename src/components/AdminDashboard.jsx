...
const AdminDashboard = () => {
  ...
  const cargarDatos = async () => {
    ...
    const { data: movs } = await supabase.from('movimientos_stock')
      .select('*, pisos(nombre_piso), pañolero:personal!movimientos_stock_dni_pañolero_fkey(jerarquia, apellido), enfermero:personal!movimientos_stock_dni_enfermero_fkey(jerarquia, apellido)')
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {personal.map(p => (
                <div key={p.dni} className="p-3 bg-slate-950 rounded-xl border border-slate-800 flex justify-between items-center group">
                  ...
                  <button onClick={() => eliminarPersonal(p.dni)} className="text-red-500 text-[10px] font-bold opacity-0 group-hover:opacity-100 transition-opacity uppercase">Eliminar</button>
                </div>
              ))}
            </div>
          </section>
