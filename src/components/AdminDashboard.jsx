...
const eliminarVisualizador = async (visId, usuario) => {
  ...
  return (
    <div className="p-6 md:p-8 bg-slate-950 min-h-screen text-slate-100 font-sans">
      ...
      {activeTab === 'log-recorridos' && (
  <div className="animate-in fade-in duration-500">
    <div className="flex justify-between items-center mb-6">
      <h2 className="text-xl font-black text-white tracking-tighter uppercase">
        Log de Recorridos <span className="text-blue-500">| Histórico</span>
      </h2>
      <button 
        onClick={cargarLogs} 
        className="text-[10px] bg-slate-800 hover:bg-slate-700 text-slate-400 px-3 py-1 rounded-lg font-bold border border-slate-700 transition-all"
      >
        🔄 ACTUALIZAR LOGS
      </button>
    </div>
    ...
    <div className="bg-slate-900/80 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl">
      <table className="w-full text-left">
        <thead className="bg-slate-800/50 text-[10px] text-blue-400 font-black uppercase tracking-widest border-b border-slate-800">
          <tr>
            <th className="p-4">Fecha y Hora</th>
            <th className="p-4">Responsable</th>
            <th className="p-4">Sector</th>
            <th className="p-4 text-center">Estado Camas</th>
          </tr>
        </thead>
            ...
            <tr key={log.id} className="hover:bg-blue-500/5 transition-colors group">
              <td className="p-4">
                <div className="text-slate-200 font-bold text-sm">
                  {new Date(log.fecha_registro).toLocaleDateString('es-AR')}
                </div>
                <div className="text-[10px] text-slate-500 font-mono">
                  {new Date(log.fecha_registro).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} HS
                </div>
              </td>
              <td className="p-4">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 bg-slate-800 rounded-full flex items-center justify-center text-[10px] font-black text-blue-500 border border-slate-700 group-hover:border-blue-500/50 transition-all">
                    {log.apellido_hist[0]}
                  </div>
                  <div>
                    <div className="text-[9px] text-blue-500 font-black uppercase tracking-tighter">
                      {log.jerarquia_hist}
                    </div>
                    <div className="text-sm text-white font-medium">
                      {log.apellido_hist}, {log.nombre_hist}
                    </div>
                  </div>
                </div>
              </td>
              <td className="p-4">
                <span className="text-xs font-bold text-slate-400 bg-slate-800/50 px-2 py-1 rounded border border-slate-700">
                  {log.pisos?.nombre_piso}
                </span>
              </td>
              <td className="p-4">
                <div className="flex justify-center gap-2">
                  <div className="text-center">
                    <div className="text-[10px] font-black text-red-500 uppercase">Ocupadas</div>
                    <div className="text-lg font-black text-white leading-none">{log.camas_ocupadas}</div>
                  </div>
                  <div className="w-[1px] bg-slate-700 mx-1"></div>
                  <div className="text-center">
                    <div className="text-[10px] font-black text-green-500 uppercase">Libres</div>
                    <div className="text-lg font-black text-white leading-none">{log.camas_libres}</div>
                  </div>
                </div>
              </td>
            </tr>
      ...
      {logs.length === 0 && (
        <div className="p-10 text-center text-slate-500 italic text-sm">
          No hay registros de recorridos en el historial...
        </div>
      )}
    </div>
  </div>
