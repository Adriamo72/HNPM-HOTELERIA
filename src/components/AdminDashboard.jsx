...
const AdminDashboard = () => {
  ...
  const [habitaciones, setHabitaciones] = useState([]); // Nueva para persistencia
  ...
  const cargarDatos = async () => {
    ...
    const resHabs = await supabase.from('habitaciones_especiales').select('*'); // Cargar habitaciones guardadas
    ...
    // --- Lógica de cálculo de stock (Mantenida idéntica al original) ---
    const stockMap = {};
    const globalAcc = {};
  ...
  const agregarHabitacionEspecial = async (pisoId, pisoSlug) => {
    const nombre = prompt("Nombre de la Habitación (Ej: Medico Interno):");
    if(nombre) {
      const slug = `${pisoSlug}-${nombre.toLowerCase().replace(/ /g, '-')}`;
      await supabase.from('habitaciones_especiales').insert([{ piso_id: pisoId, nombre: nombre, slug: slug }]);
      cargarDatos();
    }
  };
    ...
    <div className="p-4 md:p-8 bg-slate-950 min-h-screen text-slate-100 font-sans relative">
      ...
      {activeTab === 'historial' && (
        <section className="space-y-8 animate-in fade-in">
          ...
          {/* PATRIMONIO (Igual al original) */}
          <div className="bg-blue-900/10 border-2 border-blue-900/30 rounded-[2.5rem] p-6 shadow-2xl">
            <p className="text-[11px] font-black text-blue-400 uppercase tracking-[0.3em] mb-4 text-center italic">Patrimonio Total Consolidado (HNPM)</p>
          ...
          {/* TABLAS POR PISO (Igual al original) */}
          {Object.keys(resumenStock).map((nombrePiso) => (
            <div key={nombrePiso} className="bg-slate-900 rounded-[3.5rem] border border-slate-800 overflow-hidden shadow-2xl mt-8">
              <div className="bg-slate-800/40 px-8 py-4 border-b border-slate-800 text-left"><span className="text-lg font-black text-blue-400 uppercase tracking-widest">{nombrePiso}</span></div>
                ...
                {movimientosAgrupados[nombrePiso]?.map((m) => (
                  <div key={m.id} className="bg-slate-950/50 px-3 py-1.5 rounded-2xl border border-slate-800/50 flex items-center group hover:bg-slate-800 transition-all">
                    ...
                    <div className="flex-1 flex justify-around items-center px-4">
                        <div className="text-center min-w-[50px]"><span className="text-[7px] text-green-500 font-black uppercase block tracking-widest">Ingreso</span><p className="text-lg font-black text-green-500">{m.entregado_limpio > 0 ? `+${m.entregado_limpio}` : '--'}</p></div>
                    ...
                    <div className="w-[32%] flex items-center justify-end gap-3 border-l border-slate-800 pl-3">
                        ...
                        <button onClick={() => eliminarMovimiento(m.id)} className="p-1.5 bg-red-950/30 text-red-500 rounded-lg border border-red-900/30"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {activeTab === 'admin' && (
        <div className="space-y-10 animate-in fade-in text-left">
          ...
          {/* PERSONAL (Recuperado completo) */}
          <section className="bg-slate-900 p-6 rounded-[2rem] border border-slate-800 shadow-2xl">
            <h3 className="text-xs font-black text-slate-500 mb-6 uppercase tracking-widest text-left">Personal de Guardia</h3>
            <form onSubmit={agregarPersonal} className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-6 text-left">
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scroll text-left">
              {personal.map(p => (
          ...
          {/* SECTORES (Con persistencia de habitaciones) */}
          <section className="bg-slate-900 p-6 rounded-[2rem] border border-slate-800 shadow-2xl">
            <h3 className="text-xs font-black text-slate-500 mb-6 uppercase tracking-widest text-left">Sectores / Pisos</h3>
            <form onSubmit={agregarPiso} className="flex gap-2 mb-8">
              ...
              <button className="bg-blue-600 px-8 rounded-2xl font-black text-[10px] uppercase shadow-lg">Crear Sector</button>
            </form>
            <div className="grid grid-cols-1 gap-6 text-left">
              {pisos.map(p => (
                <div key={p.id} className="bg-slate-950 p-6 rounded-3xl border border-slate-800 shadow-lg">
                  ...
                  {/* LISTA DE HABITACIONES GUARDADAS */}
                  <div className="bg-slate-900/50 p-4 rounded-2xl border border-slate-800/50">
                    <div className="flex justify-between items-center mb-4">
                       ...
                       <button onClick={() => agregarHabitacionEspecial(p.id, p.slug)} className="bg-blue-600/20 text-blue-400 px-4 py-1.5 rounded-xl text-[9px] font-black uppercase border border-blue-600/30">+ Agregar</button>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                      {habitaciones.filter(h => h.piso_id === p.id).map(hab => (
                        <div key={hab.id} className="bg-slate-900 p-2 rounded-xl border border-slate-800 flex justify-between items-center">
                          <span className="text-[10px] font-bold uppercase truncate pr-2 text-slate-300">{hab.nombre}</span>
                          <div className="flex gap-1">
                            <button onClick={() => descargarQR(`/habitacion/${hab.slug}`, `${hab.nombre} - ${p.nombre_piso}`)} className="bg-slate-800 p-1.5 rounded text-[8px] text-blue-400 border border-slate-700">QR</button>
                            <button onClick={async () => { if(window.confirm("¿Eliminar?")) { await supabase.from('habitaciones_especiales').delete().eq('id', hab.id); cargarDatos(); } }} className="text-red-500 text-[10px] p-1.5">×</button>
                          </div>
                        </div>
                      ))}
                    </div>
