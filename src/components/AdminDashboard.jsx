    ...
    <div className="p-4 md:p-8 bg-slate-950 min-h-screen text-slate-100 font-sans">
      ...
      {activeTab === 'historial' && (
        <section className="space-y-10 animate-in fade-in">
          ...
          <div className="bg-blue-900/10 border-2 border-blue-900/30 rounded-[3rem] p-8 shadow-2xl">
            <p className="text-[12px] font-black text-blue-400 uppercase tracking-[0.4em] mb-6 text-center italic font-bold">Patrimonio Total Consolidado (HNPM)</p>
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
              {ITEMS_REQUERIDOS.map(item => (
                <div key={item} className="bg-slate-900 p-5 rounded-[2rem] border border-blue-800/40 text-center shadow-lg">
                  <span className="text-[10px] text-slate-500 font-black uppercase block mb-1">{item}</span>
                  <span className="text-4xl font-black text-blue-400">{stockGlobal[item] || 0}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {Object.keys(resumenStock).map((nombrePiso) => (
            <div key={nombrePiso} className="bg-slate-900 rounded-[3.5rem] border border-slate-800 overflow-hidden shadow-2xl">
              <div className="bg-slate-800/40 px-10 py-5 border-b border-slate-800">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-2 p-5 bg-slate-950/50 border-b border-slate-800">
                {ITEMS_REQUERIDOS.map(item => {
                  ...
                  return (
                    <div key={item} className={`p-5 rounded-[2rem] border transition-all ${esCero ? 'bg-red-600 border-red-400 animate-pulse text-white' : esBajo ? 'bg-red-900/20 border-red-900/50' : 'bg-slate-900 border-slate-800'}`}>
              ...
              {/* HISTORIAL DISTRIBUIDO HORIZONTALMENTE */}
              <div className="p-4 space-y-2 overflow-y-auto max-h-[450px] custom-scroll">
                {movimientosAgrupados[nombrePiso]?.map((m) => (
                  <div key={m.id} className="bg-slate-950/50 px-8 py-4 rounded-[2rem] border border-slate-800/50 flex items-center group hover:bg-slate-800/40 transition-all">
                    {/* Información de Item y Fecha */}
                    <div className="w-[20%]">
                      <p className="text-sm font-black text-white uppercase leading-none">{m.item}</p>
                      <p className="text-[10px] text-blue-500 font-black uppercase mt-1 tracking-tighter italic">
                        {formatearFechaGuardia(m.created_at)}
                      </p>
                    </div>

                    {/* BLOQUES DE FLUJO DISTRIBUIDOS (Espaciado Equitativo) */}
                    <div className="flex-1 flex justify-around items-center px-10">
                      <div className="text-center">
                        <span className="text-[9px] text-green-500 font-black uppercase block tracking-widest">Ingreso</span>
                        <p className="text-3xl font-black text-green-500">+{m.entregado_limpio || 0}</p>
                      </div>
                      <div className="text-center">
                        <span className="text-[9px] text-blue-400 font-black uppercase block tracking-widest">Entrega</span>
                        <p className="text-3xl font-black text-blue-400">-{m.egreso_limpio || 0}</p>
                      </div>
                      <div className="text-center">
                        <span className="text-[9px] text-red-500 font-black uppercase block tracking-widest">Sucio</span>
                        <p className="text-3xl font-black text-red-500">-{m.retirado_sucio || 0}</p>
                      </div>
                    </div>

                    {/* Responsables y Eliminación */}
                    <div className="w-[25%] flex items-center justify-end gap-6 border-l border-slate-800 pl-6">
                      <div className="text-right">
                        <p className="text-[11px] text-slate-300 font-black uppercase leading-tight tracking-tighter">
                      ...
                      <button 
                        ...
                        className="p-3 bg-red-950/30 text-red-500 rounded-2xl border border-red-900/30 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600 hover:text-white shadow-lg"
                      >
