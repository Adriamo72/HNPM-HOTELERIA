...
const AdminDashboard = () => {
  ...
  const cargarDatos = async () => {
    ...
    setPersonal(resPers.data || []); setPisos(resPisos.data || []); setMovimientosAgrupados(agrupados); setResumenStock(stockMap); setStockGlobal(globalFinal);
  };
  ...
  const toggleAuditoria = async () => {
    ...
    setAuditoriaHabilitada(nuevoEstado); mostrarSplash(nuevoEstado ? "MODO AUDITORÍA ACTIVADO" : "MODO AUDITORÍA CERRADO");
  };
  ...
  const agregarPersonal = async (e) => {
    ...
    setNuevoMiembro({ dni: '', nombre: '', apellido: '', jerarquia: '', celular: '', rol: 'pañolero' }); cargarDatos();
  };

  const formatearFechaGuardia = (fechaISO) => {
    const fecha = new Date(fechaISO); const opciones = { weekday: 'long', day: 'numeric' };
    const diaYNumero = fecha.toLocaleDateString('es-AR', opciones); const hora = fecha.toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' });
                  ...
                  <div className="bg-slate-900/50 p-4 rounded-2xl border border-slate-800/50">
                    <div className="flex justify-between items-center mb-4">
                       ...
                       <button onClick={() => {
                          ...
                          if(nombre) { generarQR(`/habitacion/${p.slug}-${nombre.toLowerCase().replace(/ /g, '-')}`, `${nombre.toUpperCase()} - ${p.nombre_piso}`); }
