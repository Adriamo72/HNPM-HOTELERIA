            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
              {ITEMS_REQUERIDOS.map(item => (
                <div key={item} className="bg-slate-900/80 p-4 rounded-3xl border border-blue-800/40 text-center">
                  <span className="text-[9px] text-slate-500 font-black uppercase block mb-1">{item}</span>
              ...
              <div className="p-4 space-y-2 overflow-y-auto max-h-[450px] custom-scroll">
                {movimientosAgrupados[nombrePiso]?.map((m) => (
                  <div key={m.id} className="bg-slate-950/50 px-8 py-4 rounded-[2rem] border border-slate-800/50 flex items-center group hover:bg-slate-800 transition-all">
                    ...
                    {/* FLUJO DISTRIBUIDO HORIZONTALMENTE */}
                    <div className="flex-1 flex justify-around items-center px-10">
                      <div className="text-center">
                        ...
                        <p className="text-2xl font-black text-green-500">+{m.entregado_limpio || 0}</p>
                      </div>
                      <div className="text-center">
                        <span className="text-[8px] text-blue-400 font-black uppercase block mb-1">Entrega</span>
                        <p className="text-2xl font-black text-blue-400">-{m.egreso_limpio || 0}</p>
                      </div>
                      <div className="text-center">
                        <span className="text-[8px] text-red-500 font-black uppercase block mb-1">Sucio</span>
                        <p className="text-2xl font-black text-red-500">-{m.retirado_sucio || 0}</p>
                      </div>
                    </div>

                    <div className="w-[28%] flex items-center justify-end gap-4">
                      ...
                      <button 
                        ...
                        className="p-2 bg-red-950/30 text-red-500 rounded-lg border border-red-900/30 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600 hover:text-white shadow-lg"
                      >
